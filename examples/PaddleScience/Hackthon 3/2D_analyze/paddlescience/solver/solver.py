...
class Solver(object):
    ...
    def solve(
        ...
        if paddle.in_dynamic_mode():
            return self.__solve_dynamic(num_epoch, bs, checkpoint_freq, checkpoint_path)
        else:
            if paddle.distributed.get_world_size() == 1:
    ...
    def __solve_dynamic(self, num_epoch, bs, checkpoint_freq, checkpoint_path):
        ...
        if isinstance(self.opt, paddle.optimizer.AdamW) or isinstance(
            ...
            for epoch in range(num_epoch):
                ...
                loss, outs, loss_details, loss_log = self.algo.compute(
                    None,
                    *inputs_labels,
                    ninputs=ninputs,
                ...
                print(
                    ...
                    loss.numpy()[0],
                    " eq loss:",
                    loss_details[0].numpy()[0],
                    " bc loss:",
                    loss_details[1].numpy()[0],
                    " ic loss:",
                    loss_details[2].numpy()[0],
                    " data loss:",
                ...
                if config.visualdl_enabled() == True:
                    ...
                    writer_eq_loss.add_scalar(
                        tag="detail_loss", step=epoch, value=loss_details[0].numpy()[0]
                    )
                    writer_bc_loss.add_scalar(
                        tag="detail_loss", step=epoch, value=loss_details[1].numpy()[0]
                    )
                    writer_ic_loss.add_scalar(
                        tag="detail_loss", step=epoch, value=loss_details[2].numpy()[0]
                    )
                    writer_data_loss.add_scalar(
                        tag="detail_loss", step=epoch, value=loss_details[3].numpy()[0]
                    )
            ...
            np.save(checkpoint_path + "Log_18p_pre_2k.npy", loss_logs)
            # print time
            timer.end()
            timer.print()
        ...
        # L-bfgs optimizer
            ...
            for epoch in range(num_epoch):
                ...
                print(
                    ...
                    results[3].numpy()[0],
                    " eq loss:",
                    self.loss_details[0].numpy()[0],
                    " bc loss:",
                    self.loss_details[1].numpy()[0],
                    " ic loss:",
                    self.loss_details[2].numpy()[0],
                    " data loss:",
