...
class FormulaLoss:
    ...
    def eq_loss(
        ...
        for i in range(len(pde.equations)):
            ...
            rst = cmploss.compute_formula(
                formula, input, input_attr, labels, labels_attr, None
            )
            # rst shape is flatten
            loss_log.append(rst)
            # TODO: simplify
        ...
        return loss, cmploss.outs, np.array(loss_log)

    # compute loss on one boundary
    # there are multiple bc on one boundary
    def bc_loss(
        self, pde, net, name_b, input, input_attr, labels, labels_attr, bs, params=None
    ):
    ...
    def ic_loss(
        self, pde, net, input, input_attr, labels, labels_attr, bs, params=None
    ):
    ...
    def data_loss(
        ...
        for i in range(len(pde.dvar)):
            ...
            data = labels[idx]
            loss_log.append(cmploss.outs[:, i].numpy() - data.numpy())
