...
class PINNs(AlgorithmBase):
    ...
    def create_labels_from_pde(self, pde, interior_shape=None, supervised_shape=None):
        ...
        if pde.time_disc_method is not None:
            ...
            for i in range(len(pde.dvar_n)):
                labels_attr["interior"]["data_cur"].append(LabelInt(len(labels)))
        ...
        if pde.geometry.user is not None:
            ...
            if pde.time_disc_method is not None:
                ...
                for i in range(len(pde.dvar_n)):
                    labels_attr["user"]["data_cur"].append(LabelInt(len(labels)))
                    if supervised_shape == None:
    ...
    def compute(
        self, params, *inputs_labels, ninputs, inputs_attr, nlabels, labels_attr, pde
    ):
        ...
        for name_i, input_attr in inputs_attr["interior"].items():
            ...
            loss_i, out_i, _ = self.loss.eq_loss(
                pde,
                self.net,
                input,
                input_attr,
        ...
        for name_d, input_attr in inputs_attr["user"].items():
            ...
            # eq loss
            loss_id, out_id, eq_loss_log = self.loss.eq_loss(
                pde,
                self.net,
                input,
                input_attr,
            ...
            # data loss
            loss_d, out_d, data_loss_log = self.loss.data_loss(
                pde,
                self.net,
                input,
                input_attr,
        ...
        loss_data = (loss - loss) if isinstance(loss_data, float) else loss_data
        ...
        return (
            ...
            loss_details,
            {
                "eq": eq_loss_log,
                "data": data_loss_log,
                "labels": np.array([labels[6], labels[7], labels[8]]).T,
                "out": out_d.numpy(),
                "input": input.numpy(),
            },
        )
