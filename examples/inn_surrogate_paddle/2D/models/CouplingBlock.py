...
class CouplingBlock(nn.Layer):
    ...
    def forward(self, x, c, sample_the_data=False):
        ...
        if sample_the_data == False:
            ...
            y2 = (paddle.exp(0.636 * 2 * paddle.atan(self.s_network))) * x2 + self.t_network
            output = paddle.concat((x1, y2), 1)
            jacobian2 = paddle.sum((0.636 * 2 * paddle.atan(self.s_network)), axis=tuple(range(1, self.input_len + 1)))
            ...
            self.s_network = self.s_net(x1_c)
            self.t_network = self.t_net(x1_c)
            temp = (x2 - self.t_network) / (paddle.exp(0.636 * 2 * paddle.atan(self.s_network)))
            output = paddle.concat((x1, temp), 1)
            jacobian1 = paddle.sum((0.636 * 2 * paddle.atan(self.s_network)), axis=tuple(range(1, self.input_len + 1)))
